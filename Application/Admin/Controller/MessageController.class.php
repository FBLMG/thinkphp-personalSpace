...
class MessageController extends BaseAdminController {
	...
	public function lst(){
		/*调用继承方法*/
       $this->top();                       //调取公共方法
       $this->navigation(6,1);                //调取左侧导航颜色
		/*调用继承方法*/
       //获取用户组信息
       $usergroup=M('usergroup')->select();
       $this->assign('usergroup',$usergroup);
       if(isset($_GET['groupid'])){           //查询用户组
        //
         $whereUser['groupid']=$_GET['groupid'];
         $user=M('user')->where($whereUser)->select();
         $userId=array();
         $i=0;
    ...
    //获取用户组信息
		//查询留言信息
		if(isset($_GET['status'])){      //查询未回复留言
           $where['status']=$_GET['status'];
		}

		if(isset($_GET['garbage'])){   //查询垃圾邮箱
           $where['biaozhi']=array('eq',2);
		}else{
			$where['biaozhi']=array('eq',1);
		}
		if(isset($_POST['meTitle'])){  //根据用户输入内容查询
       $where['content']=array("LIKE","%{$_POST['meTitle']}%");
		}
       ...
       if($_POST['userid']!=''){
        ...
        ////
         ...
         foreach ($isre as $k=> $val) {
           ...
           $html.="<img class='avatar' src='http://www.dinghongzx.cn/Public/img/uploads/userimg/".$isimg."' alt='' style='width:44px;height:44px;'>";
  ...
  public function removeMessage(){
      if($_GET){
		  ...
		  //删除回复留言
		  $whereMessage['mid']=$_GET['id'];
		  M('remessage')->where($whereMessage)->delete();
		  $this->redirect('Message/lst');
	  }
