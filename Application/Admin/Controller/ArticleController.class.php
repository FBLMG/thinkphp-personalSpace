...
class ArticleController extends BaseAdminController {
	...
	public function deletetype(){
      ...
      $whe['id']=$_GET['id'];
      $whes['fid']=$_GET['id'];
      M('filetype')->where($whe)->delete();
      M('file')->where($whes)->delete();
          ...
          if($status!='2'){
           ...
           //时间
			if($_POST['query_start_time'] && $_POST['query_end_time']){
				$where['publishtime'] = array('BETWEEN',array($start,$end));
			}else if($_POST['query_start_time']){
				$where['publishtime'] = array('EGT',$start);
			}else if($_POST['query_end_time']){
				$where['publishtime'] = array('ELT',$end);
			}
		}
		//文章列表循环
