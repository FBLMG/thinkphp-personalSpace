...
class UserController extends BaseAdminController {
	...
	public function lists(){
		...
		if(IS_POST){
			...
			if($_POST['query_start_time'] && $_POST['query_end_time']){
				$where['creattime'] = array('BETWEEN',array($start,$end));
			}else if($_POST['query_start_time']){
				$where['creattime'] = array('EGT',$start);
			}else if($_POST['query_end_time']){
				$where['creattime'] = array('ELT',$end);
			}
			//状态
			if($status!='2'){
				$where['status']=$status;
		...
		/**/
		$count1=M('user')->where($where)->count();
		$Page = new \Think\Page1($count1,2);// 实例化分页类 传入总记录数和每页显示的记录数
		$result=M('user')->where($where)->order('id')->page($_GET['p'].',2')->select();
		    //读取用户信息
	...
	public function adminlists(){
		...
		//查询条件
        //获取留言信息//
        $data=M('admin')->where($where)->order('id desc')->page($_GET['p'].',12')->select();
        $count1=M('admin')->where($where)->count();
